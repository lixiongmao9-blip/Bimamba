...
class AnomalyCLIPDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.hparams.load_from_features:
            from src.data.components.feature_dataset import VideoFrameDataset
        else:
            from src.data.components.video_dataset import VideoFrameDataset
    ...
    def _validate_required_paths(self):
        required_files = [
            ("annotation_file_normal", self.hparams.annotation_file_normal),
            ("annotation_file_anomaly", self.hparams.annotation_file_anomaly),
            ("annotation_file_test", self.hparams.annotation_file_test),
            ("annotation_file_temporal_test", self.hparams.annotation_file_temporal_test),
            ("labels_file", self.hparams.labels_file),
        ]
        ...
        for name, path in required_files:
            if not Path(path).is_file():
                raise FileNotFoundError(
                    ...
                    "Please check your dataset paths in configs/data/*.yaml."
                )

        frames_root = Path(self.hparams.frames_root)
        if not frames_root.is_dir():
            mode_hint = (
                "load_from_features=True expects pre-extracted features under `frames_root`."
                if self.hparams.load_from_features
                else "load_from_features=False expects extracted frames under `frames_root`."
            )
            raise FileNotFoundError(
                f"Missing required directory `frames_root`: {self.hparams.frames_root}. {mode_hint}"
            )
