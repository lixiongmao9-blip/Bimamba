...
class AnomalyCLIPModule(LightningModule):
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        if len(batch) == 4:
            ...
            image_features, labels, label, segment_size, _path = batch
        else:
    ...
    def on_validation_epoch_end(self):
        ...
        self.log("val/AUC", auc_roc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/AP", auc_pr, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/mAUC", mean_mc_auroc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/mAP", mean_mc_aupr, on_step=False, on_epoch=True, prog_bar=True)
        # Joint validation score to select checkpoints that balance binary AUC and mAUC.
        val_joint = 0.5 * (auc_roc + mean_mc_auroc)
        self.log("val/joint", val_joint, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def on_test_start(self):
        ...
        save_dir = project_root / "logs" / "train" / "runs" / str(save_dir)
    ...
    def test_epoch_end(self, outputs: List[Any]):
        ...
        save_dir = os.path.normpath(ckpt_path.parent).split(os.path.sep)[-1]
        project_root = Path(os.getenv("PROJECT_ROOT", str(Path.cwd())))
        save_dir = project_root / "logs" / "eval" / "runs" / str(save_dir)
        if not save_dir.is_dir():
            save_dir.mkdir(parents=True, exist_ok=True)
